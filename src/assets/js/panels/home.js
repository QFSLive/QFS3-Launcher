...
class Home {
        ...
        if (news) {
            if (!news.length) {
                ...
                blockNews.innerHTML = `
                    ...
                    <div class="news-content">
                        <div class="bbWrapper">
                            <p>The Federation has not issued any bulletins at this time.</p>
                        </div>
                    </div>`
            ...
            blockNews.innerHTML = `
                <div class="news-header">
                    ...
                    <div class="news-content">
                        <div class="bbWrapper">
                            <p>Uplink Failure: Unable to reach the news database.</br>Please verify your network connection.</p>
                        </div>
                    </div>`
            newsElement.appendChild(blockNews);
        }
    ...
    async startGame() {
        ...
        launch.on('check', (progress, size) => {
            infoStarting.innerHTML = `Syncing Files ${((progress / size) * 100).toFixed(0)}%`
            ipcRenderer.send('main-window-progress', { progress, size })
            progressBar.value = progress;
            progressBar.max = size;
        });
        ...
        launch.on('patch', patch => {
            console.log(patch);
            ipcRenderer.send('main-window-progress-load')
            infoStarting.innerHTML = `Applying Patch...`
        });
        ...
        launch.on('data', (e) => {
            ...
            ipcRenderer.send('main-window-progress-load')
            infoStarting.innerHTML = `Initializing Island Link...`
            console.log(e);
        })
        ...
        launch.on('close', code => {
            ...
            infoStarting.innerHTML = `Syncing Files`
