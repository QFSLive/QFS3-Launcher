...
class Settings {
            ...
            try {
                ...
                if (e.target.classList.contains('account')) {
                    popupAccount.openPopup({
                        title: 'Syncing Identity',
                        content: 'Accessing Federation records...',
                        color: 'var(--color)'
                    })
    ...
    async javaPath() {
        ...
        let configClient = await this.db.readData('configClient')
        let javaPath = configClient?.java_config?.java_path || 'Use internal Federation Java runtime';
        ...
        document.querySelector(".java-path-set").addEventListener("click", async () => {
            ...
            } else alert("Invalid Binary: Selection must be java or javaw");
        });

        document.querySelector(".java-path-reset").addEventListener("click", async () => {
            let configClient = await this.db.readData('configClient')
            javaPathInputTxt.value = 'Use internal Federation Java runtime';
